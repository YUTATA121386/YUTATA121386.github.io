...
function generateProcessLog(state, dateStr) {
  // Pick the most dramatic messages for博弈金句
  const dramaticTypes = ["REJECT", "DISPUTE", "ESCALATE", "DIRECTIVE"];
  let quoteMsgs = state.messages.filter(m => dramaticTypes.includes(m.type) || m.priority === "urgent" || m.priority === "high");
  // Also include NOTIFY messages that have strong language
  if (quoteMsgs.length < 2) {
    const notifies = state.messages.filter(m => m.type === "NOTIFY" && (m.coreInfo.includes("拒绝") || m.coreInfo.includes("失望") || m.coreInfo.includes("错误") || m.coreInfo.includes("必须")));
    quoteMsgs = [...quoteMsgs, ...notifies].slice(0, 5);
  }
  let quotes = "";
  for (const m of quoteMsgs.slice(0, 5)) {
    const fn = AGENT_NAMES_CN[m.from] || m.from;
    const tn = AGENT_NAMES_CN[m.to] || m.to;
    quotes += "> **" + fn + " → " + tn + "**: \"" + m.coreInfo + "\"\n>\n";
  }
  if (!quotes) quotes = "> 今日团队运转平稳，无激烈博弈记录\n>\n";

  const dateCN = new Date(dateStr).getFullYear() + "年" + (new Date(dateStr).getMonth() + 1) + "月" + new Date(dateStr).getDate() + "日";
  const pr = state.stats.collectorSubmitted > 0 ? ((state.stats.verifierPassed / state.stats.collectorSubmitted) * 100).toFixed(1) : "0";

  let arbSection = "";
  if (state.arbitration && state.arbitration.verdict) {
    arbSection = "\n## ⚖️ 仲裁记录\n\n### 初审\n";
    if (state.arbitration.rounds[0]?.result?.fact_list) arbSection += state.arbitration.rounds[0].result.fact_list.map(f => "- " + f).join("\n") + "\n";
    arbSection += "\n### 复审\n";
    if (state.arbitration.rounds[1]?.result?.positions) {
      for (const [k, v] of Object.entries(state.arbitration.rounds[1].result.positions)) arbSection += "- **" + (AGENT_NAMES_CN[k] || k) + "**: " + v + "\n";
    }
    arbSection += "\n### 终审\n> " + JSON.stringify(state.arbitration.verdict) + "\n";
  }

  let emergSection = "";
  if (state.emergencyChannel) emergSection = "\n## ⚡ 紧急通道\n- 触发: " + (state.emergencyChannel.triggered_by || "未知") + " | " + (state.emergencyChannel.topic || "未知") + "\n";

  const msgsLog = state.messages.map(m => {
    const fn = AGENT_NAMES_CN[m.from] || m.from;
    const tn = AGENT_NAMES_CN[m.to] || m.to;
    return "### " + m.id + "\n\n**" + fn + "** → **" + tn + "** | " + m.type + " | " + m.priority + " | " + m.timestamp + "\n\n> " + m.coreInfo + "\n\n" + (m.expectedAction ? "**期望**: " + m.expectedAction + "\n" : "") + (m.reason ? "**理由**: " + m.reason + "\n" : "");
  }).join("\n---\n");

  return "---\ntitle: " + dateStr + " | 团队过程日志\noutline: [2, 3]\n---\n\n# 📋 团队过程日志 · " + dateCN + "\n\n## 🎭 今日博弈金句\n" + (quotes || "> 今日无特殊博弈记录") + "\n## 📊 今日统计\n- 总轮次: " + state.round + " (" + (state.deadlockDetected ? "触发仲裁" : "正常") + ")\n- 采集提交: " + state.stats.collectorSubmitted + " | 通过: " + state.stats.verifierPassed + " | 通过率: " + pr + "%\n- 拒绝: " + state.stats.verifierRejected + " | 补采请求: " + state.stats.analystRequests + "\n- 紧急通道: " + (state.emergencyChannel ? "触发" : "未触发") + "\n- 规则变更: " + (state.stats.ruleChanges || 0) + " 条\n" + emergSection + arbSection + "\n## 📋 完整通信记录\n\n" + msgsLog + "\n\n---\n> 生成时间: " + new Date().toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" }) + "\n";
}
