...
function generateProcessLog(state, dateStr) {
  // Pick dramatic messages
  var dramaticTypes = ["REJECT", "DISPUTE", "ESCALATE", "DIRECTIVE"];
  var quoteMsgs = state.messages.filter(function(m) { return dramaticTypes.indexOf(m.type) >= 0 || m.priority === "urgent" || m.priority === "high"; });
  if (quoteMsgs.length < 2) {
    var notifies = state.messages.filter(function(m) { return m.type === "NOTIFY" && (m.coreInfo.indexOf("\u62d2\u7edd") >= 0 || m.coreInfo.indexOf("\u5931\u671b") >= 0 || m.coreInfo.indexOf("\u9519\u8bef") >= 0 || m.coreInfo.indexOf("\u5fc5\u987b") >= 0); });
    quoteMsgs = quoteMsgs.concat(notifies).slice(0, 5);
  }
  var quotes = "";
  for (var qi = 0; qi < Math.min(quoteMsgs.length, 5); qi++) {
    var m = quoteMsgs[qi];
  ...
  if (state.arbitration && state.arbitration.verdict) {
    ...
    if (state.arbitration.rounds[0] && state.arbitration.rounds[0].result && state.arbitration.rounds[0].result.fact_list) {
      arbSection += state.arbitration.rounds[0].result.fact_list.map(function(f) { return "- " + f; }).join("\n") + "\n";
    }
    arbSection += "\n### \u590d\u5ba1\n";
    if (state.arbitration.rounds[1] && state.arbitration.rounds[1].result && state.arbitration.rounds[1].result.positions) {
      var positions = state.arbitration.rounds[1].result.positions;
      for (var pk in positions) { arbSection += "- **" + (AGENT_NAMES_CN[pk] || pk) + "**: " + positions[pk] + "\n"; }
    }
  ...
  var msgsLog = state.messages.map(function(m) {
    var fn = AGENT_NAMES_CN[m.from] || m.from;
    var tn = AGENT_NAMES_CN[m.to] || m.to;
    var time = m.timestamp ? m.timestamp.slice(11, 16) : "";
    var lines = [];
    lines.push('<div class="chat-msg chat-from-' + m.from + '">');
    lines.push('<div class="chat-header"><strong>' + fn + '</strong> \u2192 <strong>' + tn + '</strong> &nbsp; <code>' + m.type + '</code> &nbsp; <small>' + time + '</small></div>');
