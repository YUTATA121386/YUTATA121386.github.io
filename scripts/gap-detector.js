...
r += '# Knowledge Gap Report\n\n';
r += '> Generated: ' + dateStr + '\n';
r += '> Window: ' + range + ' (' + rFiles.length + ' reports)\n';
r += '> ' + cats.length + ' categories | ' + totalKws + ' keywords\n';
r += '> ' + allFiles + ' notes scanned\n\n---\n\n';

r += '## HIGH PRIORITY GAPS\n\n';
if (high.length === 0) r += 'None found.\n\n';
else {
  r += 'These topics appear frequently in radar but have no notes.\n\n';
  for (const g of high) {
    r += '### ' + g.cat + '\n';
    r += '- Radar: **' + g.rc + '** times (last ' + LOOKBACK_DAYS + ' days)\n';
    r += '- Personal notes: none\n';
    r += '- Suggestion: ' + getAdvice(g.cat) + '\n\n';
  }
}

r += '---\n\n## MEDIUM PRIORITY\n\n';
if (mid.length === 0) r += 'None.\n\n';
else {
  for (const g of mid) {
    r += '- **' + g.cat + '** - radar ' + g.rc + 'x, notes ' + g.pc;
    if (g.pf.length > 0) r += ' (existing: ' + g.pf.join(', ') + ')';
    r += '\n';
  }
  r += '\n';
}
...
if (gf.length === 0) r += 'None.\n\n';
else {
  for (const g of gf) r += '- **' + g.cat + '** - radar ' + g.rc + 'x <-> notes ' + g.pc + '\n';
  r += '\n';
}

r += '---\n\n## FULL TABLE\n\n';
r += '| Status | Category | Radar Hits | Notes | Files |\n';
r += '|--------|----------|-----------|-------|-------|\n';
for (const g of gaps) {
  const s = g.pc >= 2 ? 'OK' : g.pc >= 1 ? '~' : 'GAP';
